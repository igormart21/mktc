document.addEventListener('DOMContentLoaded', function () {
    ...
    const hectaresInput = document.getElementById('id_hectares_atendidos');
    ...
    // Validação do campo hectares_atendidos
    if (hectaresInput) {
        hectaresInput.addEventListener('input', function(e) {
            const value = parseInt(e.target.value);
            if (isNaN(value)) {
                showError(hectaresInput, 'Por favor, digite um número válido');
            } else if (value < 10) {
                showError(hectaresInput, 'O valor mínimo é 10 hectares');
            } else if (value > 300) {
                showError(hectaresInput, 'O valor máximo é 300 hectares');
            } else {
                clearError(hectaresInput);
            }
        });
    }
    ...
    if (form) {
        form.addEventListener('submit', function (event) {
            ...
            // Validar hectares_atendidos
            if (hectaresInput) {
                const value = parseInt(hectaresInput.value);
                if (isNaN(value) || value < 10 || value > 300) {
                    event.preventDefault();
                    showError(hectaresInput, 'O valor deve estar entre 10 e 300 hectares');
                }
            }
